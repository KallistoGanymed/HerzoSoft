...
public class EKGGraph
{
    public static void main_method(String Datei) {
    ...
    public static void fileMover(String Datei) {
        ...
        File testDirectory = new File(chooseDirectory());

        File destFile = new File(testDirectory, sourceFile.getName());

        try {
            // Überprüfe, ob das Zielverzeichnis existiert, und erstelle es, falls nicht
            if (!testDirectory.getParentFile().exists()) {
                testDirectory.getParentFile().mkdirs();
            }
